...
def load_factor_and_return_from_ddb(stat_date, factor_name, config, ret_type='bid_ask_return'):
...
def do_ic_evaluate(data, factor_name):
...
def do_portforlio_rough_return(data, factor_name, date):
...
def do_ts_ic_evaluate(data, factor_name, date):
...
def process_perFactor(factor_name, start_date, end_date, config, ret_type):
    ...
    for date in pd.date_range(start_date, end_date):
        ...
        if data is not None:
            datas.append(data)
            # 时序截面的IC
            # do_ts_ic_evaluate(data, factor_name, date)
            # 计算分组的收益率
            do_portforlio_rough_return(data, factor_name, date)

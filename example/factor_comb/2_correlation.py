...
if __name__ == "__main__":
    ...
    factor_filepath = '/home/wangzirui/workspace/factor_eval_summary/param_optimized/satisfied_factors.yml'
    with open(factor_filepath, 'r') as f:
        factor_cols = yaml.load(f, Loader=yaml.FullLoader)
    ...
    # sel_cols = df.columns[:5].to_list() + factor_cols
    # df = df[sel_cols]
            
    # cols = df.columns[5:]
    
    # sort the sequence of factor by their ts IC mean
    cols = sortFactor_by_tsIC(list(factor_cols))

...
if __name__ == "__main__":
    ...
    with open(f'{base_dir}/satisfied_factors_nohl.yml', 'r') as f:
        factor_cols = yaml.load(f, yaml.FullLoader)
        
    factor_cols += ['lin_reg', 'ewc', 'xgboost']
    # factor_cols = ['lin_reg', 'ewc', 'xgboost']
    ...
    for i, factor_name in enumerate(factor_cols):
        ...
        if factor is not None:
            ...
            print(f"[warning] factor:{factor_name} is not basic factor!!!")
        ic_results = pd.concat([ic_results, factor_result], axis=0, ignore_index=True)
    
    xlsx_filepath = f"{base_dir}/ic_summary_final.xlsx"
